...
public class TACCommands implements CommandExecutor {
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    if (label.equalsIgnoreCase("tac") || label.equalsIgnoreCase("taconfig")) {
      ...
      if (args.length == 1) {
        if (Main.inputMap.containsKey(args[0].toLowerCase())) {
          sender.sendMessage(ChatColor.GREEN + "The particle " + (String)Main.caseMap.get(args[0].toLowerCase()) + 
              " is currently set to " + Main.particles.get(Main.inputMap.get(args[0].toLowerCase())));
          return true;
        } 
        if (args[0].equalsIgnoreCase("list") || args[0].equalsIgnoreCase("l")) {
          ...
          for (Map.Entry<String, Particle> p : (Iterable<Map.Entry<String, Particle>>)Main.inputMap.entrySet()) {
            ...
            if (a != 0) {
              sender.sendMessage(
                  ChatColor.GOLD + (String)Main.caseMap.get(p.getKey()) + ": " + ChatColor.RESET + a);
              inactive = false;
            } 
          } 
          if (inactive)
